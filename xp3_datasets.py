"""This module lists the datasets used as part of xP3 for training and evaluation.
...
TRAIN_DATASETS = [
    ...
    ('GEM/xlsum', "yoruba"),
    # Note that this list is expanded with flores200, wmt & more wikilingua below
]
...
TASK_HIERARCHY = {
    "question_answering_multiple_choice": [
        ("commonsense_qa", None),
        ("dream", None),
        ("quail", None),
        ("quartz", None),
        ...
        ("wiki_hop", "original"),
        ("clue", "c3"),
        # Not currently included in Figure 1
        ('ai2_arc', 'ARC-Challenge'),
        ('ai2_arc', 'ARC-Easy'),
        ('race', 'high'),
        ('race', 'middle'),
    ...
    "question_answering_closed_book": [
        ('kilt_tasks', 'hotpotqa'),
        ('wiki_qa', None),
        # Not currently included in Figure 1
        ('web_questions', None),
...
# We keep track of the datasets used by T0 to avoid duplicating the prompts reported
# in # the appendix of that work. This metadata is derived from Table 5 in
...
for l1_code in WIKILINGUA_LANGS:
    for l2_code in WIKILINGUA_LANGS:
        if l1_code == l2_code:
            continue
        dataset_name = ("GEM/wiki_lingua", f"{l1_code}_{l2_code}")
        TRAIN_DATASETS.append(dataset_name)
        LANGUAGE_STATUS_MAP["multilingual"].append(dataset_name)
        TASK_HIERARCHY["summarization"].append(dataset_name)
...
for (l1_name, l1_code) in FLORES_LANGS:
    for (l2_name, l2_code) in FLORES_LANGS:
        ...
        TRAIN_DATASETS.append(dataset_name)
        LANGUAGE_STATUS_MAP["multilingual"].append(dataset_name)
        TASK_HIERARCHY["translation"].append(dataset_name)
...
for (l1_code, l2_code) in WMT22_LANGS:
    if l1_code not in DS_TO_LANG or l2_code not in DS_TO_LANG:
        ...
        continue
    dataset_name = ("allenai/wmt22_african", f"{l1_code}-{l2_code}")
    TRAIN_DATASETS.append(dataset_name)
    LANGUAGE_STATUS_MAP["multilingual"].append(dataset_name)
    TASK_HIERARCHY["translation"].append(dataset_name)
